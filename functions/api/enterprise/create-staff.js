...
export async function onRequestPost(context) {
    ...
    try {
        ...
        const assignedAdminId = user.id; // The creator becomes the assigned admin

        // 2. Parse Request
        const body = await request.json();
        const { name, email, phone, sub_licenses } = body;
        ...
        const { results: userInsert } = await env.DB.prepare(`
            INSERT INTO users (email, password, role, enterprise_id, full_name, plan, phone, assigned_admin_id, sub_license_count)
            VALUES (?, ?, 'user', ?, ?, 'enterprise', ?, ?, ?)
            RETURNING id
        `).bind(email, password, enterpriseId, name, phone || null, assignedAdminId, sub_licenses || 0).run();
